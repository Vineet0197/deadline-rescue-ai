...
export function AuthPanel({ user, onAuthChange }: AuthPanelProps) {
  ...
  async function runEmailAuth(mode: "signin" | "signup") {
  ...
  async function startOAuth(provider: "google" | "github") {
    ...
    try {
      ...
      if (!response.ok || !data.authorizationUrl) {
        throw new Error(data.error ?? `Could not start ${provider} sign in.`);
      }
    ...
    <>
      {oauthProvider ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 rounded-2xl bg-white p-8 shadow-xl">
            ...
            <p className="mt-2 text-center text-sm text-slate-600">Please wait</p>
          </div>
        </div>
      ) : null}
      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <h3 className="font-semibold text-slate-900">Authentication</h3>
        <p className="mt-1 text-sm text-slate-600">Sign in to get started</p>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              className="rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-emerald-700 disabled:bg-slate-400"
              disabled={isAuthLoading}
              onClick={() => runEmailAuth("signin")}
              type="button"
            >
              Sign In
            </button>
            <button
              className="rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50 disabled:bg-slate-100"
              disabled={isAuthLoading}
              onClick={() => runEmailAuth("signup")}
              type="button"
            >
              Sign Up
            </button>
        ...
        {authError ? <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-800">{authError}</p> : null}
      </div>
    </>
  );
}
